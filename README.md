...
A Relational Database is a database that stores data in a table form, containing columns and rows. While SQL (Structured Query Language) is the standard language for dealing and performing data interaction tasks with Relational Databases.

- [X] Why do tables need a `primary key`?

We (and the computer) need to be able to uniquely identify each row of data (the primary key should never be null)
...
Many to Many relationships between two tables need a 'Intermediary' table - this table will have foreign keys pointing to the primary keys of the two tables, as well as a 'composite' primary key.
